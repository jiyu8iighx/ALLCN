...
+----------------------+--------------------+
	...
	另可参考: `Garrett's Links to Logical Languages <http://minyeva.alkaline.org/links.htm>`_
...
4. 真值的语义蕴含：如果对于一些乃至所有的句子模板 ``f`` ，句子 ``f(x)`` 为真时 ``f(y)`` 也总是为真，那么我们认为 ``x`` 语义蕴含了 ``y`` ，逻辑上一般记作 :math:`x\vDash y` 。一个词可以真蕴含于多个词。
